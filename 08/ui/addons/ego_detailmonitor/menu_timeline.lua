...
function menu.createTimelineTable(frame)
	...
	for _, entry in ipairs(menu.timeline) do
		...
		if isknown then
			if C.IsKnownItemRead("timeline", entry.id) then
				...
				row[4]:createText(ColorText["text_positive"] .. "(" .. ReadText(1001, 2449) .. ")", { wordwrap = true, halign = "right" })
			end
		end
	...
	Helper.drawLine( { x = ftable.properties.x + lineoffsetX, y = ftable.properties.y + lineoffsetY }, { x = lineend.x, y = lineend.y }, linethickness, nil, Color["row_separator_white"], true )

	Helper.drawLine( { x = lineend.x - lineoffsetX, y = lineend.y - lineoffsetX }, { x = lineend.x + math.floor(linethickness / 2), y = lineend.y + math.floor(linethickness / 2) }, linethickness, nil, Color["row_separator_white"], true )
	Helper.drawLine( { x = lineend.x, y = lineend.y }, { x = lineend.x + lineoffsetX, y = lineend.y - lineoffsetX }, linethickness, nil, Color["row_separator_white"], true )
...
function menu.createRendertargetAndDescription(frame, timelineTable)
	...
	if not menu.hasRendertarget then
		local row = ftable:addRow(false, { fixed = true, bgColor = Color["frame_background_semitransparent"] })
